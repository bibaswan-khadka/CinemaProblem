...
def test_input1():
    ...
    assert filecmp.cmp('./InputOutputs/input1.txt.output','./Solutions/input1.txt.output',shallow=False) == True
    ...
    with open('./InputOutputs/input2.txt', "r") as input_file:
            ...
            with open('./InputOutputs/input2.txt' + '.output' , "w") as output_file:
                ...
                for i in seating_assignments:
                    formatted_text += i + '\n'
                output_file.write(formatted_text)
    assert filecmp.cmp('./InputOutputs/input2.txt.output','./Solutions/input2.txt.output',shallow=False) == True
